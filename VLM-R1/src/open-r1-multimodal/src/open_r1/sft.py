    ...
    def __getitem__(self, i):
        ...
        def make_conversation_image(example):
            ...
            if "solution" in example and "problem" in example:
                ...
                normal_caption = example.get("normal_caption", "object")
                return [
                    {
                        "role": "user",
                        "content": [
            ...
            if "conversations" in example:
                ...
                assistant_turn = next(turn for turn in example["conversations"] if turn["from"] == "gpt")
                user_text = human_turn["value"].replace("<image>", "").strip()
                bbox = assistant_turn["value"]
                x1, y1, x2, y2 = bbox
                return [
                    {
                        "role": "user",
            ...
            raise KeyError(f"Unsupported REC example format: {example.keys()}")
